...
# Optimize the wasm
if [ "$DUMP_WASM" == "1" ]; then
    wasm2wat $file -o $example_dir/wasm.unoptimized.wat
fi
...
wasm-opt -S -g --enable-simd -O4 --dce --inlining-optimizing -iit -lmu --optimize-added-constants-propagate --vacuum $OPTS $file -o $example_dir/wasca.wat
wat2wasm $example_dir/wasca.wat -o $example_dir/wasca.wasm


if [ "$DUMP_WASM" == "1" ]; then
wasm2wat $file -o $file.optimized
wasm2wat $example_dir/wasca.wasm -o $example_dir/wasm.wat
fi
