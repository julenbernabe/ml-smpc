...
int main(int argc, const char *argv[])
{
  ...
  opt.add("",                         // Default.
          ...
          1,                          // Number of args expected.
          0,                          // Delimiter if expecting multiple args.
          "Override input file path", // Help description.
          "-in",                      // Flag token.
          "-path_in"                  // Flag token.
  );
  opt.add("",                          // Default.
          ...
          1,                           // Number of args expected.
          0,                           // Delimiter if expecting multiple args.
          "Override output file path", // Help description.
          "-out",                      // Flag token.
          "-path_out"                  // Flag token.
  );
  ...
  opt.get("-in")->getString(path_in);
  opt.get("-out")->getString(path_out);
  ...
  main_logic(pns, minimums, maximums, maxI, my_number, verbose, fhefacts, progname, memtype, portnumbase, OT_disable, path_in, path_out, nullptr, nullptr);
}
#endif
    ...
    void *api_handle)
{
  ...
  if (path_in.empty())
    {
      path_in= "Data/Player" + std::to_string(my_number) + "_in.txt";
    }
  if (path_out.empty())
    {
      path_out= "Data/Player" + std::to_string(my_number) + "_out.txt";
    }
  ...
  if (api_constructor)
  ...
  else
    {
      ...
      auto iof= std::make_unique<Input_Output_Simple>();
      cout << "\nUsing Input_Output_Simple" << endl;

      string fsharein_str= "Data/Player" + std::to_string(my_number) + "_sharein.txt";
      string fshareout_str= "Data/Player" + std::to_string(my_number) + "_shareout.txt";

      struct stat stats;
      int r= fstat(fileno(stdin), &stats);
      if (r >= 0 && S_ISFIFO(stats.st_mode))
        {
          // Input from pipe
          cout << "Processing pipe input..." << endl;
          iof->init(my_number, SD.n, path_out, fsharein_str, fshareout_str, false);
        }
      else
        {
          // Input from file (argument)
          cout << "Processing file (argument) input..." << endl;
          iof->init(my_number, SD.n, path_in, path_out, fsharein_str, fshareout_str, false);
        }
